...
| 3 | `segment_fit` | Classification enum (see above) |
| 4 | `company_stage` | Classification enum |
| 5 | `vertical` | Classification enum |
| 6 | `ai_adoption` | Classification enum |
| 7 | `regulatory_exposure` | Classification enum |
| 8 | `size_fit` | Classification enum |
| 9 | `buyer_name` | "Other row fields to fill in directly" |
| 10 | `buyer_title` | "Other row fields to fill in directly" |
| 11 | `buyer_accessibility` | Classification enum |
| 12 | `wrong_fit_risk` | Classification enum (boolean) |
| 13 | `startup_stigma_routing` | Classification enum |
| 14 | `score_total` | `scoring.py score` output — `score_total` field |
| 15 | `score_breakdown` | `scoring.py score` output — **use the `score_breakdown_str` value (the compact `"k=v;k=v"` string), NOT the `score_breakdown` field (which is a JSON object/dict).** Putting the dict here defeats the column's purpose (a compact breakdown for retuning rubric weights later) and Python's CSV writer will stringify it as `"{'segment_fit': 30, ...}"`. |
| 16 | `tier` | `scoring.py score` output — `tier` field |
| 17 | `reachability_notes` | "Other row fields to fill in directly" |
| 18 | `rationale` | "Other row fields to fill in directly" |
| 19 | `sources` | "Other row fields to fill in directly" |
| 20 | `confidence` | "Other row fields to fill in directly" |
| 21 | `first_seen` | Auto-managed by `csv_store.py` — do not send this; it's set/preserved on upsert |
| 22 | `last_researched` | Auto-managed by `csv_store.py` — do not send this; it's set on every upsert |
| 23 | `status` | "Other row fields to fill in directly" |
| 24 | `outcome` | Leave empty (`""`) — filled in manually by the sales team later, not by this skill |
